  ...
  return (
    <div className="flex flex-col">
      ...
      {link.hasDropdown && link.subLinks && (
        <div
          className={`overflow-hidden transition-all duration-300 ease-in-out flex flex-col pl-6 pr-3 border-l-[1.5px] border-[#fff7e8] ml-4 space-y-1 ${
            isDropdownOpen ? "max-h-[400px] py-2 mt-1 opacity-100" : "max-h-0 py-0 mt-0 opacity-0"
          }`}
        >
    ...
    <div className={`relative w-full border-b border-[#eee8df] bg-white ${className || ''}`}>
      ...
      <div
        className={`lg:hidden overflow-hidden transition-all duration-300 ease-in-out ${
          open ? "max-h-[1000px] opacity-100 border-t border-[#eee8df]" : "max-h-0 opacity-0"
        }`}
      >
